        ...
        foreach (var body in data)
        {
            ...
            if (body.IsTracked)
            {
                if (!Bodies.ContainsKey(body.TrackingId))
                {
                    ...
                    Quaternion x_var = Quaternion.Euler(Camera.main.transform.forward );
                    Vector3 pos = dude.transform.localPosition;
                    Debug.Log(x_var);
                    
                    GameObject newBody = new GameObject();
                    KinectPlayer newPlayer = Instantiate(Resources.Load("BlockPlayer") as GameObject, transform.TransformPoint(pos), x_var ).GetComponent<KinectPlayer>();
                    newPlayer.transform.parent = dude.transform;
                    //newPlayer.transform.Rotate(x_var);
